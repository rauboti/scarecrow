...
function initItem(item) {
  ...
  if (!$.isEmptyObject(item.stats.magical)) {
    ...
    if (item.stats.magical.dmg) { $('#txtMagicalStats').append('<div>' + item.stats.magical.dmg + ' damage / healing (IV: ' + (item.stats.magical.dmg*item.coefficients.splpwr) + ')</div>'); addToLV('#txtLVMagical', item.stats.magical.dmg, item.coefficients.splpwr); }
