...
function initItem(item) {
  ...
  if (!$.isEmptyObject(item.stats.magical)) {
    ...
    if (item.stats.magical.dmg_nature) { $('#txtMagicalStats').append('<div>' + item.stats.magical.dmg_nature + ' nature spell damage (IV: ' + (item.stats.magical.dmg_nature*item.coefficients.splpwr) + ')</div>'); addToLV('#txtLVMagical', item.stats.magical.dmg_nature, item.coefficients.splpwr); }
  ...
  if (!$.isEmptyObject(item.stats.physical)) {
    if (item.stats.physical.ap) { $('#txtPhysicalStats').append('<div>' + item.stats.physical.ap + ' melee attack power (IV: ' + (item.stats.physical.ap*item.coefficients.ap) + ')</div>'); addToLV('#txtLVPhysical', item.stats.physical.ap, item.coefficients.ap); }
    if (item.stats.physical.rap) { $('#txtPhysicalStats').append('<div>' + item.stats.physical.rap + ' ranged attack power (IV: ' + (item.stats.physical.rap*item.coefficients.rap) + ')</div>'); addToLV('#txtLVPhysical', item.stats.physical.rap, item.coefficients.rap); }
