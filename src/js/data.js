...
const local = module.exports = {
  ...
  character: {
    ...
    delete: async function(character, user) {
      const result = await sql.query('DELETE FROM tblCharacter WHERE id = ? AND user_id = ?', [character, user]);
      return;
    },
    set: {
  ...
  event: {
    ...
    get: async function(id, user) {
      ...
      debug('Refreshed main character successfully!')
      ...
      debug('Event loaded successfully!')
      ...
      debug('Signups loaded successfully!')
      debug('Iterating through the signups..')
      for (var i in result) {
  ...
  user: {
    ...
    set: {
      details: async function(user, id, admin) {
        if (admin) {
          const result = await sql.query('UPDATE tblUser SET user = ?, rank = ?, role = ?, email = ? WHERE id = ?', [user.username, user.rank, user.role, user.email, id]);
        } else {
  ...
  users: {
    getAll: async function() {
      const result = await sql.query('SELECT u.id, u.user, u.rank as "rankid", r.name as "rank", u.role FROM tblUser u JOIN tblRank r on r.id = u.rank ORDER BY u.rank DESC, u.user ASC');
      return result;
    },
